...
<style>
    ...
    /* Le Titre Principal (H1) centré et gros */
    .main-title {
        text-align: center;
...
st.markdown("### L'Anti-Agenda : L'ingénierie IA au service de votre Neuro-Type.")

# --- L'ARGUMENTAIRE SCIENTIFIQUE (Le Manifesto) ---
st.markdown("""
<div class="concept-box">
    <h4 style="color:#FF4B4B; margin-bottom:15px;">🛑 Le Mensonge de la Productivité Moderne</h4>
    <p style="font-size:15px; line-height:1.6;">
        <b>Pourquoi Notion, Trello et Google Calendar vous épuisent ?</b><br>
        Parce qu'ils ont été conçus par et pour des profils "Ingénieurs" (Haute Conscience, Faible Névrosisme). Ils imposent une structure linéaire rigide à des cerveaux qui fonctionnent parfois en arborescence, en hyperfocus ou par impulsions.
        <br><br>
        Si vous n'arrivez pas à tenir un agenda, <b>vous n'êtes pas "bordélique" ou "paresseux".</b> Vous êtes simplement victime d'une <i>incompatibilité biologique</i>. Vous essayez de faire tourner un logiciel Windows sur un Mac : ça surchauffe.
    </p>
    <hr style="border-color:#333; margin:20px 0;">
    <h4 style="color:#00ff00; margin-bottom:15px;">🧬 Notre Solution : L'Isomorphisme Cognitif</h4>
    <p style="font-size:15px; line-height:1.6;">
        Nous ne cherchons pas à vous "réparer". Nous adaptons la structure temporelle à votre chimie cérébrale via deux moteurs :
        <br><br>
        1. 🧪 <b>Le Profilage OCEAN (Big Five) :</b> C'est le standard d'or de la psychométrie moderne. Nous cartographions vos 5 traits dominants pour comprendre votre coût énergétique réel.
        <br>
        2. 🤖 <b>Le "Psychometric Prompt Engineering" :</b> Contrairement à un ChatGPT générique, notre IA change de personnalité pour s'aligner sur la vôtre. Elle devient stricte si vous avez besoin de cadre, ou ludique si vous avez besoin de dopamine.
    </p>
</div>
""", unsafe_allow_html=True)
...
# COLONNES COMPACTES (VISUALISATION)
...
with c1:
    ...
    <div class="profile-example">
        <strong style="font-size:18px;">👤 Profil A : "Le Soldat"</strong><br>
        <span style="color:#aaa;">(Haute Conscience, Basse Ouverture)</span>
        <hr style="margin:12px 0; border-color:#444;">
        <p style="color:#aaffaa; font-weight:bold;">✅ Stratégie IA "Architecte" :</p>
        <div style="text-align:left; font-size:16px; color:#ddd; line-height:1.5;">
        • Planning séquentiel (9h-18h)<br>
        • Pauses fixes de 15 min<br>
        • <b>Objectif :</b> Constance & Rigueur
        </div>
        <br>
        <i style="color:#666; font-size:14px;">"L'IA agit comme un Chef de Projet."</i>
    </div>
    """, unsafe_allow_html=True)
    
with c2:
    st.markdown('<div class="versus" style="display:flex; align-items:center; justify-content:center; height:100%; font-weight:bold; color:#555;">VS</div>', unsafe_allow_html=True)
    
with c3:
    st.markdown("""
    <div class="profile-example">
        <strong style="font-size:18px;">👤 Profil B : "L'Artiste"</strong><br>
        <span style="color:#aaa;">(Haute Ouverture, Basse Conscience)</span>
        <hr style="margin:12px 0; border-color:#444;">
        <p style="color:#ffaaaa; font-weight:bold;">✅ Stratégie IA "Coach Sprint" :</p>
        <div style="text-align:left; font-size:16px; color:#ddd; line-height:1.5;">
        • Blocs "Deep Work" de 4h (Flow)<br>
        • Zéro contrainte le matin (Liberté)<br>
        • <b>Objectif :</b> Intensité & Dopamine
        </div>
        <br>
        <i style="color:#666; font-size:14px;">"L'IA agit comme un Agent Artistique."</i>
    </div>
    """, unsafe_allow_html=True)
...
with st.form("psycho_form"):
    ...
    with col_brain:
        ...
        with st.container(border=True):
            ...
            st.markdown("""
            <div style="font-size:16px; line-height:1.4; color:#ddd; margin-bottom:10px;">
...
# --- SECTION 3 : LE CIBLAGE STRATÉGIQUE (Remplacement Complet) ---
    ...
    with st.container(border=True):
        ...
        # --- ETAPE 1 : LE VECTEUR D'INTENSITÉ ---
        st.markdown('<div style="color:#FF4B4B; font-weight:bold; margin-bottom:5px;">🌡️ Étape 1 : Calibrer l\'Intensité (Phase de Vie)</div>', unsafe_allow_html=True)
        
        # Création des colonnes : Input à gauche, Théorie à droite
        c_life_input, c_life_theory = st.columns([1.5, 1], gap="large")
        
        with c_life_input:
            st.caption("Sélectionnez la doctrine tactique qui s'applique aujourd'hui.")
            life_phase = st.radio(
                "Phase de Vie",
                [
                    "🔥 **War Time (Combat)** : Urgence critique. La mission est unique, tout le reste est distraction. Je sacrifie temporairement l'équilibre et le confort pour une victoire rapide. Sommeil militaire, focus laser, zéro mondanités. C'est un sprint de survie.",
                    
                    "🏗️ **Builder (Construction)** : Phase de création d'actif (Coder une app, écrire un livre). J'exige des blocs de 'Deep Work' ininterrompus de 4h le matin. L'administratif et le social sont relégués en fin de journée. Priorité au 'Maker Time'.",
                    
                    "⚖️ **Maintenance (Équilibre)** : Phase de gestion (Salarié, Parents). L'objectif est la durabilité. Je cherche à optimiser le ratio effort/résultat sans m'épuiser. Time-boxing strict (9h-18h) pour protéger ma vie personnelle.",
                    
                    "🌱 **Recovery (Récupération)** : Post-Burnout ou Vacances. Mon système nerveux est à bout. Priorité absolue à la santé mentale, au sommeil et à la déconnexion. Je veux réduire la charge cognitive au minimum vital."
                ],
                label_visibility="collapsed"
            )

        with c_life_theory:
            st.info("🎛️ **Théorie : La Modulation du Prompt**")
            st.markdown("""
            **Pourquoi ce choix change tout ?**
            Deux profils identiques (ex: Architecte / Lion) auront deux plannings opposés selon la phase de vie. Ce réglage modifie l'agressivité de l'IA.
            
            **⚙️ Mécanique de Calibrage :**
            * **En War Time :** L'IA supprime les "Buffers", condense les pauses, et utilise un ton impératif. Elle autorise le dépassement horaire.
            * **En Recovery :** L'IA double les temps de pause, interdit les tâches complexes après 16h et priorise le sommeil.
            
            **💡 Exemple Concret :**
            Pour une tâche de 2h :
            * *War Time* -> "Bloc de 1h45 intensif + 15min ravitaillement."
            * *Recovery* -> "2x 45min doux + 30min marche nature."
            """)
        
        st.markdown("---")

        # --- ETAPE 2 : LE VORTEX (BRAIN DUMP) ---
        st.markdown('<div style="color:#00ff00; font-weight:bold; margin-bottom:10px;">🌪️ Étape 2 : Le Vortex (Videz votre RAM)</div>', unsafe_allow_html=True)
        
        c_dump, c_dump_theo = st.columns([1.5, 1], gap="large")
        
        with c_dump:
            st.caption("Ne triez pas. Notez tout ce qui encombre votre cerveau (Tâches, Urgences, Idées). L'IA fera le tri.")
            mission = st.text_area(
                "Brain Dump", 
                placeholder="- Finir le rapport Python\n- Acheter du pain\n- Appeler maman\n- Réviser le chapitre 3\n- Urgence : Payer facture EDF...",
                height=200,
                label_visibility="collapsed"
            )
            
            # VERSION SIMPLE (Juste le slider standard)
            st.markdown("**Fenêtre de Tir : décris ta disponibilité pour le planning de l'IA**")
        ...
        with c_dump_theo:
            st.info("🧠 **Théorie : La Décharge Cognitive**")
            st.markdown("""
            **Pourquoi le "Vrac" est vital ?**
            Votre cerveau n'est pas fait pour *stocker* des idées (RAM), mais pour en *traiter* (Processeur). Garder une liste de tâches en tête consomme une énergie colossale (Effet Zeigarnik).
            
            **🤖 Le Rôle du Filtre IA :**
            L'IA va prendre votre liste brute et la filtrer à travers votre **Phase de Vie** et votre **Profil Neuro**.
            * *War Time* : Elle supprime "Acheter du pain" et met "Rapport Python" en priorité absolue.
            * *Maintenance* : Elle intercale les tâches domestiques dans vos creux d'énergie.
            """)
        
        st.markdown("---")

        # --- ETAPE 3 : LE DIAGNOSTIC DE FRICTION (AUTOPSIE) ---
        st.markdown('<div style="color:#4DA6FF; font-weight:bold; margin-bottom:10px;">🚧 Étape 3 : Identifier l\'Ennemi (Le "Pourquoi")</div>', unsafe_allow_html=True)
        
        c_frict_input, c_frict_theo = st.columns([1.5, 1], gap="large")
        
        with c_frict_input:
            st.caption("Pour que l'IA construise vos défenses, soyez précis. Inspirez-vous des listes ci-dessous pour diagnostiquer vos failles.")
            
            # --- ZONE 1 : INTERNE ---
            st.markdown("**👹 Sabotage Interne (Vos boucles mentales)**")
            
            # La "Carte Menu" (Non cliquable, juste visuelle)
            st.markdown("""
            <div style="background-color:rgba(255, 75, 75, 0.1); padding:10px; border-radius:5px; font-size:16px; color:#aaa; margin-bottom:10px; border-left:3px solid #FF4B4B;">
                <b>🎯 Pistes de diagnostic pour le sabotage interne :</b><br>
                • <b>L'Inertie :</b> "Je ne sais pas par où commencer, la tâche semble montagneuse."<br>
                • <b>Le Perfectionnisme :</b> "J'ai peur que le résultat soit médiocre, alors je repousse."<br>
                • <b>L'Ennui (Dopamine) :</b> "La tâche est trop répétitive, je décroche vite."<br>
                • <b>La Fatigue :</b> "Brouillard mental après 14h, impossible de réfléchir."
                <b>📡 Pistes de diagnostic pour le sabotage externe:</b><br>
                • <b>Le Numérique :</b> "Mon téléphone vibre (Slack/Insta), réflexe pavlovien."<br>
                • <b>L'Humain :</b> "Collègues/Famille qui m'interrompent sans cesse."<br>
                • <b>Le Sonore :</b> "Open space bruyant ou travaux à côté."<br>
                • <b>L'Imprévu :</b> "Urgences qui tombent et cassent mon planning."
            </div>
            """, unsafe_allow_html=True)
            
            friction_internal = st.text_area(
                "Interne",
                placeholder="Décrivez votre blocage ici (Ex: J'ai peur de mal faire, alors je vais sur YouTube...)",
                height=100,
                label_visibility="collapsed",
                key="frict_int"
            )
            ...
            # --- ZONE 2 : EXTERNE ---
            st.markdown("**🛡️ Siège Externe (Environnement)**")
            
            # La "Carte Menu"
            st.markdown("""
            <div style="background-color:rgba(77, 166, 255, 0.1); padding:10px; border-radius:5px; font-size:16px; color:#aaa; margin-bottom:10px; border-left:3px solid #4DA6FF;">
                <b>📡 Pistes de diagnostic pour le sabotage externe:</b><br>
                • <b>Le Numérique :</b> "Mon téléphone vibre (Slack/Insta), réflexe pavlovien."<br>
                • <b>L'Humain :</b> "Collègues/Famille qui m'interrompent sans cesse."<br>
                • <b>Le Sonore :</b> "Open space bruyant ou travaux à côté."<br>
            ...
            friction_external = st.text_area(
                "Externe",
                placeholder="Décrivez votre environnement ici (Ex: Mon téléphone est une extension de ma main...)",
                height=100,
                label_visibility="collapsed",
                key="frict_ext"
            )
        ...
        with c_frict_theo:
            st.info("🧠 **Théorie : Les Intentions d'Implémentation**")
            st.markdown("""
            **Pourquoi détailler l'échec ?**
            En psychologie comportementale, pour vaincre une mauvaise habitude, il faut identifier le **Déclencheur (Trigger)** précis.
            
            **La Stratégie "Si... Alors..." :**
            Si vous dites juste *"Distraction"*, l'IA ne peut rien faire.
            Si vous avouez *"Je prends mon téléphone quand je stresse"*, l'IA peut programmer une contre-mesure tactique :
            
            > *"Protocole : À 10h00 (Tâche complexe), mettre le téléphone dans une autre pièce (Friction physique) pour couper le réflexe dopaminergique."*
            
            C'est la différence entre un conseil bienveillant et un plan de bataille.
            """)
...
# --- LOGIQUE DE SOUMISSION (BACKEND V5) ---
if submitted:
    # 1. Consolidation des Scores
    # On récupère directement les valeurs des sliders (o_score, c_score...)
    final_scores = {
        "Ouverture": o_score, "Conscience": c_score, 
        "Extraversion": e_score, "Agréabilité": a_score, "Névrosisme": n_score
    }

    # 2. Calcul de la Tendance (Profil Macro)
    if c_score >= 75:
    ...
    # 3. Validation
    # On vérifie juste que le Brain Dump (mission) n'est pas vide
    if not mission:
        st.warning("⚠️ Le Brain Dump est vide. Donnez de la matière à l'IA pour travailler.")
        st.stop()

    # 4. Construction de la Payload (C'est ici qu'on connecte la War Room)
    inputs = {
        "scores": final_scores,
        "work_style": {
        ...
        "context": {
            # --- LES NOUVEAUX CHAMPS V5 ---
            "phase_de_vie": life_phase,       # War Time / Maintenance...
            "mission": mission,               # Le "Vrac" (Brain Dump)
            "horaires": f"{time_range[0]}h00 - {time_range[1]}h00", # Le Slider Temps
            "frictions_internes": friction_internal, # Le diagnostic texte interne
            "frictions_externes": friction_external  # Le diagnostic texte externe
        }
    }

    # 5. Exécution & Stockage
    with st.spinner("🔄 Initialisation du Core Gemini... Analyse du Vrac..."):
        # Appel au backend
        ...
        try:
            ...
            # On sauvegarde aussi les scores pour le Paywall et les graphiques
...
if 'analysis_result' in st.session_state:
    ...
    # --- FIN DES ONGLETS (On sort des tabs ici) ---
    st.markdown("---")

    # =================================================================
    # ZONE PAYWALL GLOBALE (Visible quel que soit l'onglet choisi)
    # =================================================================

    # 1. LE TEASER DE L'ANALYSE (HOOK)
    analysis_teaser = data.get('analysis_global', "Analyse du profil neuro-cognitif en cours...")
    if len(analysis_teaser) > 300:
        analysis_teaser = analysis_teaser[:300]

    # Rendu HTML (Aligné à gauche dans les guillemets)
    st.markdown(f"""
<div style="border: 1px solid #333; background-color: #161924; padding: 20px; border-radius: 10px; margin-bottom: 25px; position: relative; overflow: hidden; min-height: 220px;">
<h4 style="color: #4DA6FF; margin-top: 0; margin-bottom: 10px; font-size: 18px; display: flex; align-items: center; gap: 10px;">
<span>🧠</span> Analyse Stratégique (Extrait)
</h4>
<p style="color: #ddd; font-size: 15px; line-height: 1.6; margin: 0; filter: blur(0.5px);">
{analysis_teaser}...
</p>
<div style="position: absolute; bottom: 0; left: 0; width: 100%; height: 140px; display: flex; flex-direction: column; justify-content: center; align-items: center; background: linear-gradient(to bottom, rgba(22, 25, 36, 0) 0%, rgba(22, 25, 36, 0.95) 40%, #161924 100%);">
<div style="border: 1px solid #FF4B4B; background-color: rgba(14, 17, 23, 0.95); padding: 15px 25px; border-radius: 8px; text-align: center; box-shadow: 0 0 30px rgba(255, 75, 75, 0.15);">
<div style="display: flex; justify-content: center; align-items: center; gap: 10px;">
<div style="font-size: 20px;">🔒</div>
<span style="color: white; font-weight: 800; font-size: 13px; text-transform: uppercase; letter-spacing: 1px;">DÉBLOQUER L'AUDIT COMPLET (FORCES & FAILLES)</span>
</div>
</div>
</div>
</div>
""", unsafe_allow_html=True)
    ...
    # 2. LE TEASER DU PLANNING (PROOF + FRUSTRATION)
    st.markdown("### 🗓️ Protocole d'Exécution (Aperçu Tactique)")
    ...
    if len(planning) > 0:
        ...
        # A. On affiche les 3 premières lignes claires
        df_teaser = df_free.head(3).copy()
        
        if "start_iso" in df_teaser.columns:
            df_teaser["Heure"] = df_teaser["start_iso"].apply(lambda x: x.split('T')[1][:5] if 'T' in x else x)
        else:
            df_teaser["Heure"] = "N/A"

        df_display = df_teaser[["Heure", "titre", "categorie", "description"]]
        df_display.columns = ["🕒 Heure", "⚔️ Mission", "🏷️ Mode", "ℹ️ Consignes"]
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "⚔️ Mission": st.column_config.TextColumn(width="medium"),
                "🏷️ Mode": st.column_config.TextColumn(width="small"),
                "ℹ️ Consignes": st.column_config.TextColumn(width="large"),
            }
        )
        
        # 2. LA ZONE "CLASSIFIÉE" (VERSION EXPERT - COLLÉE À GAUCHE)
        st.markdown("""
<div style="background-color: #0e1117; border: 1px dashed #333; border-top: none; padding: 0; position: relative; overflow: hidden; margin-top: -1px; min-height: 360px;">
<div style="display: flex; align-items: center; padding: 12px 10px; border-bottom: 1px solid #222; opacity: 0.4; filter: blur(1.5px);">
<div style="width: 15%; color: #666; font-family: monospace;">14:15</div>
<div style="width: 30%; color: #aaa; font-weight: bold; font-family: monospace;">CYCLE D'HYPER-FOCUS (BLOC 2)</div>
<div style="width: 15%; color: #4DA6FF; font-family: monospace;">High-Beta</div>
<div style="width: 40%; color: #555; font-family: monospace;">Verrouillage sensoriel : ████████████</div>
</div>
<div style="display: flex; align-items: center; padding: 12px 10px; border-bottom: 1px solid #222; opacity: 0.3; filter: blur(2.5px);">
<div style="width: 15%; color: #666; font-family: monospace;">16:45</div>
<div style="width: 30%; color: #aaa; font-weight: bold; font-family: monospace;">PROTOCOLE DE RESYNCHRONISATION</div>
<div style="width: 15%; color: #00ff00; font-family: monospace;">Neuro-Reset</div>
<div style="width: 40%; color: #555; font-family: monospace;">Technique NSDR : █████████████████</div>
</div>
<div style="display: flex; align-items: center; padding: 12px 10px; border-bottom: 1px solid #222; opacity: 0.2; filter: blur(3.5px);">
<div style="width: 15%; color: #666; font-family: monospace;">19:30</div>
<div style="width: 30%; color: #aaa; font-weight: bold; font-family: monospace;">DÉCHARGE MÉMOIRE DE TRAVAIL</div>
<div style="width: 15%; color: #FF4B4B; font-family: monospace;">System Flush</div>
<div style="width: 40%; color: #555; font-family: monospace;">Vidange Cortisol : ████████████</div>
</div>
<div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; background: linear-gradient(180deg, rgba(14,17,23,0) 0%, rgba(14,17,23,0.95) 40%, #0e1117 100%);">
<div style="border: 1px solid #FF4B4B; background-color: rgba(14, 17, 23, 0.95); padding: 25px 0; width: 85%; max-width: 700px; border-radius: 12px; text-align: center; box-shadow: 0 0 60px rgba(255, 75, 75, 0.2);">
<div style="display: flex; justify-content: center; align-items: center; gap: 15px; margin-bottom: 15px;">
<div style="font-size: 24px;">🔒</div>
<h3 style="color: white; margin: 0; font-size: 18px; text-transform: uppercase; letter-spacing: 2px; font-weight: 800;">Accès Expert Requis</h3>
</div>
<div style="text-align: left; display: inline-block; margin-bottom: 25px; color: #ccc; font-size: 15px; line-height: 1.9; background: rgba(255, 255, 255, 0.03); padding: 15px 25px; border-radius: 6px; border: 1px solid #333;">
<div style="display:flex; align-items:center; gap:8px;">✅ <b>Planning Tactique Complet</b> (4 Semaines de protocoles)</div>
<div style="display:flex; align-items:center; gap:8px;">✅ <b>Planning en fichier .ICS Universel</b> (Import direct Google/Apple/Outlook)</div>
<div style="display:flex; align-items:center; gap:8px;">✅ <b>Audit Neuro-Stratégique</b> (Analyse de vos failles cognitives, Forces et Faiblesses)</div>
</div>
<br>
<a href="https://buy.stripe.com/00w7sN5ZW5gp9GggtP0RG00" target="_blank" style="background-color: #FF4B4B; color: white; text-decoration: none; padding: 14px 40px; border-radius: 6px; font-weight: 800; font-size: 15px; display: inline-block; transition: all 0.3s; box-shadow: 0 4px 20px rgba(255, 75, 75, 0.4); text-transform: uppercase; letter-spacing: 0.5px;">DÉBLOQUER MAINTENANT (9.90€)</a>
<div style="margin-top: 15px; font-size: 15px; color: #666; font-style: italic;">Accès immédiat • Paiement sécurisé Stripe • Reçu sous 12h • Satisfait ou remboursé</div>
</div>
</div>
</div>
""", unsafe_allow_html=True)
